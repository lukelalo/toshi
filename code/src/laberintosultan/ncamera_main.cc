...
nCamera::nCamera() :
    ...
    cameraStyle(nCamera::LOCKED_CHASE),
    targetPosition(),
	camaraActual(NULL)
{
	...
	posicionEstatica.set(660, 120, 330);
	destinoEstatico.set(660, 0, 380);
...
nCamera::SetStyle(nCamera::Style style)
{
	...
	default: {	// TODO: resto de cámaras??
		...
		diferencia.norm();
		inicioCamara = posActual + diferencia * (distancia - 10.0);
...
vector3 CTrackingCamera::getNuevaPos(const vector3 &origenActual,
	...
	float distanciaDeseada = 10.0f;	 // TODO: pasarlo a atributo?
	...
	diferencia *= avanceNecesario * 1.0;//0.5f;//1.0f;
	resultado.x += diferencia.x;
	resultado.z += diferencia.z;

	// TODO: la altura se hace "a mano"
	resultado.y += ((posicionEntidad.y + 4) - resultado.y) * 0.1f;
