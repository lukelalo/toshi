...
nNpkDirectory::Open(const char* dirName)
{
    ...
    this->npkFileServer->ManglePath(dirName, mangledPath, sizeof(mangledPath));
    this->npkFileServer->MakeAbsolute(mangledPath, this->apath, sizeof(this->apath));

    this->tocEntry = this->npkFileServer->FindFirstTocEntry(this->apath);
...
nNpkDirectory::SetToNextNpkEntry()
{
    ...
    if (0 == this->curSearchEntry)
    {
        ...
        this->tocEntry = this->npkFileServer->FindNextTocEntry(this->tocEntry, this->apath);
        if (this->tocEntry)
        {
            this->curSearchEntry = this->tocEntry->GetFirstEntry();
        }
