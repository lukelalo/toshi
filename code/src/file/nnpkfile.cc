...
nNpkFile::Open(const char* filename, const char* accessMode)
{
	...
	this->npkFileServer->ManglePath(filename, mangledPath, sizeof(mangledPath));
	this->npkFileServer->MakeAbsolute(mangledPath, absPath, sizeof(absPath));

    this->tocEntry = this->npkFileServer->FindFirstTocEntry(absPath);
