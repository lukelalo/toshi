...
nMLoader::nMLoader() : nVisNode(),
	...
	rutaAttach(""), numeroAttach (-1), scale(0.01f) {
...
//	ReleaseAll();
	char buf[N_BUFFERSIZE];
	// nString nombre;
	// Establecemos el nuevo nombre.
	//rsrc_path.Set(refFileServer.get(), nombreFichero, refFileServer->GetCwd());
	refFileServer->ManglePath(nombreFichero,buf,N_BUFFERSIZE);
	rsrc_path.Set(refFileServer.get(),nombreFichero,"");
	fileName = buf;
	loadMDL();
}
...
*/
	if (_instance->getNumMeshes() > 0) {
		...
		for (unsigned int i = 0; i < _instance->getNumMeshes(); i++) {
			...
			if (tex_array[i])
				vb->Render(ref_ibuf[i].get(), shader , tex_array[i]);  //sceneGraph->GetPixelShader()
			else
...
bool nMLoader::cacheTextures() {
	...
	for (unsigned int i = 0; i < _instance->getNumMeshes(); i++) {
		...
		std::string path=rsrc_path.GetPath();
		std::string path2=rsrc_path.GetAbsPath();
		int pos=path2.find_last_of('/');
		path2=path2.substr(0,pos);
		pos=path.find_first_of(':')+1;
		if (pos)
			path=path.substr(pos,path.length());
		else {
			pos=path.find_first_of('/')+1;
			if (pos)
				path=path.substr(pos,path.length());
		...
		sprintf(buf, "%d@%s",i, path.c_str());
		n_assert(strlen(buf) < N_BUFFERSIZE);
		cambiado = refFileServer->ChangeDir(path2.c_str());
...
bool nMLoader::HasAnimationFinished() 
{
	return _instance->hasSequenceFinished();
}
...
bool nMLoader::AttachModel(int numAttach, const std::string &ruta) {
	...
	if ((numAttach < 0) || (numAttach >= (int)_instance->getNumAttachments()))
		return false;
