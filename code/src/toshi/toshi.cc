...
const char* startupScriptName = "toshi:scripts/startup.tcl";
...
#ifdef __WIN32__
    nWin32LogHandler* lh = new nWin32LogHandler("toshi");
    ks->AddLogHandler(lh);
#endif
    nFileLogHandler* fh = new nFileLogHandler("toshi");
    ks->AddLogHandler(fh);
    
    nFileServer2* fs = ks->GetFileServer2();
    fs->SetAssign("toshi", "home:data/toshi/");
    nScriptServer* ss = (nScriptServer*)ks->New("ntclserver", 
                                                "/sys/servers/script");
    const char* result = NULL;

    char buf[N_MAXPATH];
    fs->ManglePath("toshi:scripts/startup.tcl", buf, N_MAXPATH);
