#define N_IMPLEMENTS nLenseFlare22

//==============================================================================
//  node/nLenseFlare22_main.cc
...
//------------------------------------------------------------------------------
void
nLenseFlare2::BeginFlares(int iNumFlares)
{
    ...
    nFlareDesc2 tmp;
    aFlares = new nFlareDesc2[iNumberOfFlares];
...
//------------------------------------------------------------------------------
void
nLenseFlare2::SetFlareSizeAt(int ndx, float size)
{
    n_assert(bBegun == true);
...
//------------------------------------------------------------------------------
void
nLenseFlare2::SetBaseColor(float red, float grn, float blu, float alpha)
{
    n_assert(bBegun == true);
    fBaseRed   = red;
    fBaseGreen = grn;
...
//------------------------------------------------------------------------------
void
nLenseFlare2::SetBlindColor(const vector4& vBlindClr)
{
    n_assert(bBegun == true);
...
//------------------------------------------------------------------------------
void
nLenseFlare2::SetFlarePosAt(int ndx, float pos)
{
    n_assert(bBegun == true);
    n_assert(aFlares != 0 && ndx < iNumberOfFlares);
...
//------------------------------------------------------------------------------
void
nLenseFlare2::SetFlareColorAt(int ndx,
                             float red,
                             float grn,
                             float blu,
                             float alpha)
...
//------------------------------------------------------------------------------
void
nLenseFlare2::GetBaseColor(float& red,
                          float& grn,
                          float& blu,
                          float& alpha)
{
...
//------------------------------------------------------------------------------
vector4
nLenseFlare2::GetBlindColor() const
{
   return vBlindColor;
}
//==============================================================================
...
//------------------------------------------------------------------------------
void
nLenseFlare2::GetFlareColorAt(int ndx,
                             float& red,
                             float& grn,
                             float& blu,
                             float& alpha)
...
//------------------------------------------------------------------------------
bool
nLenseFlare2::Attach(nSceneGraph2* sceneGraph)
{
    n_assert(sceneGraph);
...
//------------------------------------------------------------------------------
void
nLenseFlare2::Compute(nSceneGraph2* sceneGraph)
{
    n_assert(sceneGraph);
...
nLenseFlare2::InitVBuffer()
{
    ...
    nIndexBuffer *ibuf = this->ref_gs->FindIndexBuffer("nLenseFlare2_ibuf");
    if (!ibuf)
    {
        ibuf = this->ref_gs->NewIndexBuffer("nLenseFlare2_ibuf");
...
//------------------------------------------------------------------------------
void
nLenseFlare2::UVFlares()
{
    //  assert precondition(s)
    n_assert(vb_dest);

    vector2 a1(1.0f, 1.0f); // top right
    vector2 a2(0.5f, 1.0f); // mid top
    vector2 a3(0.5f, 0.5f); // mid mid
    vector2 a4(1.0f, 0.5f); // mid top

    vector2 b1(0.5f, 1.0f);
    vector2 b2(0.0f, 1.0f);
    vector2 b3(0.0f, 0.5f);
    vector2 b4(0.5f, 0.5f);

    vector2 c1(1.0f, 0.5f);
    vector2 c2(0.5f, 0.5f);
    vector2 c3(0.5f, 0.0f);
    vector2 c4(1.0f, 0.0f);
    
    vector2 d1(0.5f, 0.5f);
    vector2 d2(0.0f, 0.5f);
    vector2 d3(0.0f, 0.0f);
    vector2 d4(0.5f, 0.0f);
    ...
    for (int i = 0; i < iNumberOfFlares * 4; i += 4, iCnt++)
    {
		...
		{
			case 1:
				vb_dest->Uv(i    , 0, a1); // uv upper right
				vb_dest->Uv(i + 1, 0, a2); // uv up
				vb_dest->Uv(i + 2, 0, a3); // uv origin
				vb_dest->Uv(i + 3, 0, a4); // uv right
				break;
			case 2:
				vb_dest->Uv(i    , 0, b1); // uv upper right
				vb_dest->Uv(i + 1, 0, b2); // uv up
				vb_dest->Uv(i + 2, 0, b3); // uv origin
				vb_dest->Uv(i + 3, 0, b4); // uv right
				break;
			case 3:
				...
				break;
			case 0:
				vb_dest->Uv(i    , 0, d1); // uv upper right
				vb_dest->Uv(i + 1, 0, d2); // uv up
				vb_dest->Uv(i + 2, 0, d3); // uv origin
				vb_dest->Uv(i + 3, 0, d4); // uv right
				break;
		}
    }
}
