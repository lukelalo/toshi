...
//-------------------------------------------------------------------
/**
    Does a visibility test on each element in the node and
    collects it if it is visible. To check visibility, 
    this->vp_matrix must be up-to-date!

    - 02-Jun-99   floh    created
*/
//-------------------------------------------------------------------
void nOctree::collect_nodes_in_viewvol(nOctNode *on)
{
    nOctElement *oe;
    for (oe = (nOctElement *) on->elm_list.GetHead();
         oe;
         oe = (nOctElement *) oe->GetSucc())
    {
        int c = this->box_clip_viewvol(oe->p0,oe->p1);
        if (c >= 0) {
            oe->SetCollectFlags(nOctElement::N_COLLECT_VIEWVOL);
            this->collect(oe);
        }
    }
}
...
//-------------------------------------------------------------------
/**
    Low level version of CollectByViewvol(), recurses as far into
    the tree as needed.

    - 02-Jun-99   floh    created
*/
//-------------------------------------------------------------------
void nOctree::recurse_collect_by_viewvol(nOctNode *on)
{
    // Clip status of the current node...
    int c = this->box_clip_viewvol(on->p0,on->p1);
    if (c > 0) {
        // Node is entirely contained, all elements of
        // the node and all of its children can be
        // trivially collected...
        this->recurse_collect_nodes_with_flags(on,nOctElement::N_COLLECT_VIEWVOL);
        
    } else if (c == 0) {
        // Node is partly inside the view volume. A bounding box
        // check is done on all elements of the node before they
        // get collected. recurse_collectbyview() is applied to
        // all child nodes.
        this->collect_nodes_in_viewvol(on);
        if (on->c[0]) {
            int i;
            for (i=0; i<8; i++) this->recurse_collect_by_viewvol(on->c[i]);
        }
    }
}
...
//-------------------------------------------------------------------
/**
    @brief Collect the nodes that are within the view volume.

    Collects the elements contained in the current view volume.
    Requires the current modelview matrix as well as the current
    projection matrix.

    Global coordinates in the octree are passed through the
    modelview and projection matrices, resulting in 4D clip
    coordinates. The clip volume is defined by
  
      - -w <= x <= w
      - -w <= y <= w
      - -w <= z <= w

    Note that it may be preferable to use the newer methods that
    use clipping planes, such as CollectByFrustum(), as performance
    should be improved.
  
    - 02-Jun-99   floh    created
    - 18-Aug-00   floh    now fills externally provided element array
*/
//-------------------------------------------------------------------
int nOctree::CollectByViewvol(matrix44& mv, 
                               matrix44& p,
                               ...
                               int size)
{
    ...
    this->recurse_collect_by_viewvol(this->tree_root);
...
*/
int nOctree::CollectByFrustum(nGfxServer* gfx_server,
                              nOctElement** ext_array,
                              int size)
{
    n_assert(ext_array);
...
*/
void nOctree::init_clip_planes_for_frustum(nGfxServer* gfx_server)
{
    float minx, maxx, miny, maxy, minz, maxz;
    gfx_server->GetViewVolume(minx, maxx, miny, maxy, minz, maxz);
